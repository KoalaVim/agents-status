#!/usr/bin/env sh
# Sourced by hook scripts to send events to the agents-status server.
# Callers set AGENTS_STATUS_DIR before sourcing; this fallback handles direct invocation.

AGENTS_STATUS_DIR="${AGENTS_STATUS_DIR:-$(cd "$(dirname "$0")" && pwd)}"

# Remove asdf shims so git/python3/tmux resolve to real binaries.
# No-op when asdf isn't installed or shims aren't on PATH.
