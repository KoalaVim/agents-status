...
def _load_config():
    """Load config from $AGENTS_STATUS_CONFIG or ~/.config/agents-status/config.toml."""
    ...
    if not os.path.isfile(path):
        return {}
    if tomllib is None:
        _log("config: tomllib unavailable, skipping %s", path)
        return {}
    try:
        with open(path, "rb") as f:
            cfg = tomllib.load(f)
        _log("config: loaded %s", path)
        return cfg
    except Exception as e:
        _log("config: failed to load %s: %s", path, e)
        return {}
...
def apply_state(instance_id, state):
    ...
    post_event_cmds = _config.get("post-event", {}).get("commands", [])

    def _spawn_followups():
        for cmd_str in post_event_cmds:
            try:
